...
def get_dominant_idx(bw: np.ndarray, verts: np.ndarray, faces: np.ndarray) -> np.ndarray:
    is_torch = False
    ...
    if faces is None or faces.shape[0] < 1000:
        dominant_idx = np.argmax(bw, axis=-1)

    else:
        N, K = bw.shape
        assert N == len(verts)
        dominant_idx = np.zeros(N, dtype=int)
        k = 6

        import scipy.sparse as sp

        # 1. 构建 1-ring 邻接矩阵 (包含自环)
        row, col = [], []
        for face in faces:
            row.extend([face[0], face[0], face[1], face[1], face[2], face[2]])
            col.extend([face[1], face[2], face[0], face[2], face[0], face[1]])

        # 使用 coo_matrix 高效构建
        adj_matrix = sp.coo_matrix((np.ones(len(row)), (row, col)), shape=(N, N))

        # 加上自环 (每个顶点都是自己的邻居)，并转换为布尔值防止重复边
        # tocsr() 格式适合矩阵乘法
        adj_matrix = (adj_matrix + sp.eye(N, format="coo")).tocsr()
        adj_matrix.data[:] = 1.0  # 确保所有存在的值都为1

        # 2. 计算 k-ring 邻接矩阵 A_k
        # A_k 是 A_1 的 k 次方 (在布尔代数意义下)
        # 我们从 A_1 开始，然后乘以 A_1 共 k-1 次
        k_ring_adj = adj_matrix.copy()

        # 这里的循环次数是 k-1，非常小
        for _ in range(k - 1):
            k_ring_adj = k_ring_adj @ adj_matrix
            # 将结果转回布尔矩阵，防止数值爆炸，并保持稀疏性
            k_ring_adj.data[:] = 1.0

        # 3. 向量化投票：一次稀疏矩阵乘法
        influence_scores = k_ring_adj @ bw

        # 4. 获取最终结果
        dominant_idx = np.argmax(influence_scores, axis=1)

    if is_torch:
        dominant_idx = torch.from_numpy(dominant_idx).unsqueeze(0)
    return dominant_idx
...
def bw_post_process(
    ...
    if no_fingers:
        dominant_idx = get_dominant_idx(bw, verts, faces)
        for hand in hands:
            bw[
                get_conflict_mask(
                    dominant_idx,
...
@spaces.GPU
@torch.no_grad()
def model_forward_coarse(pts: torch.Tensor, pts_normal: torch.Tensor) -> torch.Tensor:
    pts = pts.to(device)
    pts_normal = pts_normal.to(device)
    with torch.amp.autocast("cuda", enabled=model_coarse.ae_type == "hy3d2.1"):
        joints = model_coarse.forward(torch.cat([pts, pts_normal], dim=-1)).joints.float()
    return joints.cpu()
...
def preprocess(db: DB):
    ...
    with Timing(msg="Joints localization done in", print_fn=gr.Info):
        joints = model_forward_coarse(pts, pts_normal)
...
def model_forward_bw(
    ...
    for verts_, verts_normal_ in zip(verts_chunks, verts_normal_chunks):
        ...
        with torch.amp.autocast("cuda", enabled=model_bw.ae_type == "hy3d2.1"):
            bw_ = model_bw.forward(torch.cat([pts, pts_normal], dim=-1), torch.cat([verts_, verts_normal_], dim=-1)).bw
        bw.append(bw_.float())
...
@spaces.GPU
@torch.no_grad()
def model_forward_bones(pts: torch.Tensor, pts_normal: torch.Tensor) -> tuple[torch.Tensor]:
    pts = pts.to(device)
    pts_normal = pts_normal.to(device)
    inputs = torch.cat([pts, pts_normal], dim=-1)
    with torch.amp.autocast("cuda", enabled=model_joints.ae_type == "hy3d2.1"):
        joints = model_joints.forward(inputs).joints.float()
    with torch.amp.autocast("cuda", enabled=model_pose.ae_type == "hy3d2.1"):
        pose = model_pose.forward(inputs, joints=joints.clone() if model_pose.pose_input_joints else None).pose_trans
...
def infer(input_normal: bool, db: DB):
    ...
    with Timing(msg="Model inference done in", print_fn=gr.Info):
        bw = model_forward_bw(verts, verts_normal, pts, pts_normal)
        joints, pose = model_forward_bones(pts, pts_normal)
...
def vis(bw_fix: bool, bw_vis_bone: str, no_fingers: bool, restore_global: bool, db: DB):
    verts = db.verts
    bw = db.bw
    joints = db.joints
    pose = db.pose
    ...
    def _get_bone_loc(
        bone_xyz: torch.Tensor, bone_name: str, dim="y", type="tail", bone_idx_dict: dict[str, int] = BONES_IDX_DICT
    ):
    ...
    if pose is not None:
        ...
        if restore_global:
            T_mat = db.global_transform.get_matrix().transpose(-1, -2).squeeze(0).cpu().numpy()
            T_inv_mat = transform_inv.get_matrix().transpose(-1, -2).squeeze(0).cpu().numpy()
            pose = np.einsum("ij,kjl,lm->kim", T_inv_mat, pose, T_mat)

    if restore_global:
        verts = transform_inv.transform_points(verts)
        joints[..., :3] = transform_inv.transform_points(joints[..., :3])
        joints[..., 3:] = transform_inv.transform_points(joints[..., 3:])
        if db.gs is not None:
            db.gs = transform_gs(db.gs, transform_inv)

    bw = bw.squeeze(0).cpu().numpy()
    verts = verts.squeeze(0).cpu().numpy()
    db.mesh.vertices = verts
    vis_weights(verts, bw, db.faces, vis_bone_index=BONES_IDX_DICT[f"{MIXAMO_PREFIX}{bw_vis_bone}"]).export(db.bw_path)
    joints, joints_tail = joints.squeeze(0)[..., :3].cpu().numpy(), joints.squeeze(0)[..., 3:].cpu().numpy()
    vis_joints(verts, joints, db.faces, bones_idx_dict=BONES_IDX_DICT).export(db.joints_path)
    ...
    if pose is not None:
        ...
        if db.gs is None:
            ...
            vis_joints(
                apply_transform(verts, lbs_transform), rest_joints, db.faces, bones_idx_dict=BONES_IDX_DICT
...
def get_pose_ignore_list(pose: str = None, pose_parts: list[str] = None):
    ...
    if pose:
        if pose == "T-pose":
            kw_list.extend(
                ["Spine", "Neck", "Head", "Shoulder", "Arm", "ForeArm", "Hand", "UpLeg", "Leg", "Foot", "ToeBase"]
            )  # all
        elif pose == "A-pose":
            kw_list.extend(
                ["Spine", "Neck", "Head", "ForeArm", "Hand", "UpLeg", "Leg", "Foot", "ToeBase"]
            )  # except for Shoulder & Arm
        elif pose == "大-pose":
            kw_list.extend(
                ["Spine", "Neck", "Head", "Shoulder", "Arm", "ForeArm", "Hand", "Leg", "Foot", "ToeBase"]
            )  # except for UpLeg
    if pose_parts:
...
def _pipeline(
    ...
    time.sleep(0.1)
    yield vis(bw_fix, bw_vis_bone, no_fingers, restore_global, db)
    time.sleep(0.1)
    yield vis_blender(
        reset_to_rest,
        no_fingers,
...
def init_models():
    global device, N, hands_resample_ratio, geo_resample_ratio, model_bw, model_joints, model_coarse, model_pose
                    ...
                    with gr.Row():
                        with gr.Accordion("Weight Settings", open=False):
                            ...
                            input_bw_vis_bone = gr.Radio(
                                [n.lstrip(MIXAMO_PREFIX) for n in (BONES_IDX_DICT).keys()],
            ...
            def pipeline(inputs: dict, progress=gr.Progress()):
                ...
                yield from progress.tqdm(
                    _pipeline(
                        ...
                        restore_global=inputs[input_restore_global],
