                ...
                for param in named_parameters:
                    ...
                    if param.grad is not None:
                        ...
                        if not valid_gradients:
                            break
                if not valid_gradients:
                    name = f"{name} {tuple(param.shape)}" if name else str(tuple(param.shape))
                    print(f"Detected Inf or NaN values in gradients ({name}). Not updating model parameters.")
                    # optimizer.zero_grad()
                    return None
...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, model_only=False):
...
def load_model(args, model_without_ddp, optimizer=None, loss_scaler=None, model_preprocess=None, strict=True):
    if args.resume:
        ...
        model_without_ddp.load_state_dict(model_state_dict, strict=strict)
